...
export const LIVE_RESET_CONFIRMATION = "RESET-FTSLINE-LIVE";

export async function getLiveResetScope() {
...
async function resetCollections({ survivorId, session }) {
  ...
  const results = await Promise.all([
    ...
    User.deleteMany({ _id: { $ne: survivorId } }, options),
  ]);
...
export async function executeLiveDataReset(confirmation) {
  if (confirmation !== LIVE_RESET_CONFIRMATION) {
    throw new Error("Gecersiz temizleme onayi.");
  }

  const before = await getLiveResetScope();
  const session = await mongoose.startSession();
  let deleted;

  try {
    await session.withTransaction(async () => {
      deleted = await resetCollections({
        survivorId: before.survivor.id,
        session,
      });
    });
  } finally {
  ...
  const after = await getLiveResetScope();
  return { survivor: before.survivor, before: before.counts, deleted, after: after.counts };
}
