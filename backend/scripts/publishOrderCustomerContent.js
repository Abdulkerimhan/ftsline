...
if (!identifier || !password) throw new Error("Akademi yonetici bilgileri zorunludur.");
...
const content = JSON.parse(await fs.readFile(new URL("../data/orderCustomerContent.json", import.meta.url), "utf8"));
...
const course = courses.find((item) => {
  ...
  return title.includes("sipariş") || title.includes("siparis") || title.includes("müşteri") || title.includes("musteri");
});
if (!course) throw new Error("Siparis ve Musteri Yonetimi kursu bulunamadi.");
const result = await request(`/academy/admin/courses/${course._id}`, {
  method: "PUT", headers,
  body: JSON.stringify({
    ...course, title: content.title, description: content.description,
    ...
    lessons: content.lessons.map((lesson) => ({
      ...lesson, documentUrl: "/documents/FTSLine_Siparis_ve_Musteri_Yonetimi_Ders_Notlari.pdf",
    })),
  }),
});
