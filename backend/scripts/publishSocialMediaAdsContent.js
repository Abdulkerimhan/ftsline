...
const content = JSON.parse(await fs.readFile(
  new URL("../data/socialMediaAdsContent.json", import.meta.url), "utf8"
));
...
const course = courses.find((item) => {
  ...
  return title.includes("sosyal") || title.includes("reklam");
});
if (!course) throw new Error("Sosyal Medya ve Reklam kursu bulunamadi.");
...
const result = await request(`/academy/admin/courses/${course._id}`, {
  ...
  body: JSON.stringify({
    ...
    lessons: content.lessons.map((lesson) => ({
      ...lesson,
      documentUrl: "/documents/FTSLine_Sosyal_Medya_ve_Reklam_Ders_Notlari.pdf",
    })),
  }),
});
